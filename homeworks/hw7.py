# Ryan Monaghan
# I pledge my honor I abided by the Stevens Honor System.
...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        return lORL(s[1:], result, curCount, curVal) # TO DO (my solution is about 6 lines)
...
def findRunBits(s):
    ...
    print(s)
    lORL = listOfRunLengths(s)
    print(lORL)
    runBits = math.ceil(math.log(sum(lORL) / len(lORL))) + (min(lORL) if min(lORL) != max(lORL) else 1)
    print(runBits)
    return runBits if runBits <= 7 else 7
