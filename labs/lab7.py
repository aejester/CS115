...
# Name: Ryan Monaghan
# Pledge: I pledge my honor I have abided by the Stevens Honor System.s
...
def FA(x,y,cin):
    ...
    return (gor(gand(XOR(x, y), cin), gand(x, y)), XOR(XOR(x, y), cin))
...
def testFA():
    ...
    print("testFA successful on 8 out of 8 cases")
...
def fourBitAdd(xxxx,yyyy):
    ...
    (c1, zt) = FA(xxxx[2], yyyy[2], c)
    (c2, zf) = FA(xxxx[1], yyyy[1], c1)
    (c4, ze) = FA(xxxx[0], yyyy[0], c2)
    return (c4, (ze, zf, zt, zo))

# EXERCISE: implement the following.
def test_fourBitAdd():
    '''Test the fourBitAdd function'''
    zero = (0,0,0,0)
    one = (0,0,0,1)
    five = (0,1,0,1)
    thirteen = (1,1,0,1)
    ...
    c, ww = fourBitAdd(five,thirteen)
    assert( ww == (0,0,1,0) and c == 1 )
    c, ww = fourBitAdd(five,five)
    assert( ww == (1,0,1,0) and c == 0 )