...
def search(L, i, x):
    ...
    for j in range(1, i):
        if L[j - 1] <= x < L[j] and 0 <= j <= i:
            return j
    return i if L[i-1] < x else 0
...
def insertV2(L, i):
    ...
    x = L[i]
    index = search(L, i, x) 
    for j in list(reversed(range(index, i))):
        if L[i] < L[j]:
            swap(L, i, j)
            i = j
        else:
...
def letterCounts(fname):
    ...
    for c in contents:
        if letter(c) not in counts_dict:
            counts_dict[letter(c)] = 1
        else:
            counts_dict[letter(c)] += 1
    L = [(value, key) for key, value in counts_dict.items()]

    # inefficient but that is ok, we only have ot sort worst case 27 items.
